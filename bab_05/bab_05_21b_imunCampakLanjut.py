...
berkasData = currentdir +'\\bab_05_21_dataImunLanjut.csv'
berkasSimpan = currentdir +'\\bab_05_21b_imunCampakLanjut.pdf'
# judulDiagram = 'Cakupan Imunisasi Campak 2\nTahun 2021'
sumbuX = 'Cakupan'
...
# read data file
colnames = ['puskesmas','dptLanL','dptLanP','dptLanLP','campakLanL','campakLanP','campakLanLP']
data = pandas.read_csv(berkasData, names=colnames, sep=';')
puskesmas = data.puskesmas.tolist()
bar1 = data.dptLanL.tolist()
bar2 = data.dptLanP.tolist()
bar3 = data.dptLanLP.tolist()
bar4 = data.campakLanL.tolist()
bar5 = data.campakLanP.tolist()
bar6 = data.campakLanLP.tolist()
...
# make bars
fig, ax = plt.subplots()
rects1 = ax.barh(ind, bar4, width, color='steelblue', label=labelRects1)
rects2 = ax.barh(ind + width, bar5, width, color='orangered', label = labelRects2)
rects3 = ax.barh(ind + 2*width, bar6, width, color='yellowgreen', label = labelRects3)
