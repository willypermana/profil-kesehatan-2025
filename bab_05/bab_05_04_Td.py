...
berkasData = currentdir +'\\bab_05_04_dataTd.csv'
berkasSimpan = currentdir +'\\bab_05_04_Td.pdf'
# judulDiagram = 'Cakupan Td Ibu Hamil\nTahun 2021'
sumbuX = 'Cakupan'
sumbuY = 'Puskesmas/ Kabupaten'
tickerSumbuX = np.arange(0,110,25)

# read data file
colnames = ['puskesmas','Td5','Td2plus']
data = pandas.read_csv(berkasData, names=colnames, sep=';')
puskesmas = data.puskesmas.tolist()
bar1 = data.Td5.tolist()
bar2 = data.Td2plus.tolist()
...
# make bars
fig, ax = plt.subplots()
rects1 = ax.barh(ind, bar1, width, color='steelblue', label = 'Td5')
rects2 = ax.barh(ind + width, bar2, width, color='orangered', label = 'Td2+')
