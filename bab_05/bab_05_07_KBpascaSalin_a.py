...
berkasData = currentdir +'\\bab_05_07_dataKBpascaSalin.csv'
...
bar2 = data.Suntik.tolist()
bar3 = data.Pil.tolist()
bar4 = data.AKDR.tolist()
...
# make bars
fig, ax = plt.subplots()
rects1 = ax.barh(ind, bar1, width, color='steelblue', label='Kondom')
rects2 = ax.barh(ind + width, bar2, width, color='orangered', label = 'Suntik')
rects3 = ax.barh(ind + 2*width, bar3, width, color='green', label = 'Pil')
rects4 = ax.barh(ind + 3*width, bar4, width, color='violet', label = 'AKDR')
...
ax.legend(fontsize='x-small', loc='upper center', bbox_to_anchor=(0.5, -0.15), fancybox=True, shadow=True, ncol=4)
