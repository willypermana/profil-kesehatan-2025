...
berkasData = currentdir +'\\bab_06_15a_dataIVA.csv'
berkasSimpan = currentdir +'\\bab_06_15a_IVA.pdf'
# judulDiagram = 'Cakupan Pemeriksaan IVA\nTahun 2022'
sumbuX = 'Cakupan'
sumbuY = 'Puskesmas/ Kabupaten'
tickerSumbuX = np.arange(0,110,25)
labelBar1 = 'Pemeriksaan IVA'
labelBar2 = 'IVA+'

# read data file
colnames = ['puskesmas','tesIVA','IVApositif']
data = pandas.read_csv(berkasData, names=colnames, sep=';')
puskesmas = data.puskesmas.tolist()
bar1 = data.tesIVA.tolist()
bar2 = data.IVApositif.tolist()
...
for i, v in enumerate(bar2):
    ax.text(v+0.5, i+0.4, locale.format_string("%.2f", v), ha='left', va='center', fontsize='x-small')
